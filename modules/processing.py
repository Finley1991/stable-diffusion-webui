...
class StableDiffusionChatProcessing:
    """
    The first set of paramaters: sd_models -> do_not_reload_embeddings represent the minimum required to create a StableDiffusionProcessing
    """

    def __init__(self, sd_model=None, outpath_samples=None, outpath_grids=None,
                 prompt: str = "", styles: List[str] = None, seed: int = -1, subseed: int = -1,
                 subseed_strength: float = 0, seed_resize_from_h: int = -1, seed_resize_from_w: int = -1,
                 seed_enable_extras: bool = True, sampler_name: str = None, batch_size: int = 1, n_iter: int = 1,
                 steps: int = 50, cfg_scale: float = 7.0, width: int = 512, height: int = 512,
                 restore_faces: bool = False, tiling: bool = False, do_not_save_samples: bool = False,
                 do_not_save_grid: bool = False, extra_generation_params: Dict[Any, Any] = None,
                 overlay_images: Any = None, negative_prompt: str = None, eta: float = None,
                 do_not_reload_embeddings: bool = False, denoising_strength: float = 0, ddim_discretize: str = None,
                 ...
                 sampler_index: int = None, script_args: list = None, user_select_style: str = "写实"):
        if sampler_index is not None:
            print("sampler_index argument for StableDiffusionProcessing does not do anything; use sampler_name",
                  file=sys.stderr)
        ...
        self.outpath_samples: str = outpath_samples
        self.outpath_grids: str = outpath_grids
        self.parameter_for_chat: dict = self.get_parameter_by_user_select_style(user_select_style, prompt, width, height)
        self.prompt: str = self.parameter_for_chat.get("prompt", "")
        self.prompt_for_display: str = None
        self.negative_prompt: str = self.parameter_for_chat.get("negative_prompt", "")
        self.styles: list = styles or []
        self.seed: int = seed
        self.subseed: int = subseed
        self.subseed_strength: float = subseed_strength
        self.seed_resize_from_h: int = seed_resize_from_h
        self.seed_resize_from_w: int = seed_resize_from_w
        self.sampler_name: str = self.parameter_for_chat.get("sampler_name", "Euler")
        self.batch_size: int = batch_size
        self.n_iter: int = n_iter
        self.steps: int = self.parameter_for_chat.get("steps", 20)
        self.cfg_scale: float = self.parameter_for_chat.get("cfg_scale", 7)
        self.width: int = self.parameter_for_chat.get("width", 512)
        self.height: int = self.parameter_for_chat.get("height", 512)
        self.restore_faces: bool = restore_faces
        self.tiling: bool = tiling
        self.do_not_save_samples: bool = do_not_save_samples
        self.do_not_save_grid: bool = do_not_save_grid
    ...
    # TODO
    def get_parameter_by_user_select_style(self, user_select_style, prompt, width, height):
        parameters = {}
        if user_select_style == "写实":
            # 构造chatGPT请求参数，让chatGPT生成写实风格图片的prompt
            post_data = {"user_style_select": user_select_style,
                         "user_question": prompt,
                         "language_select_dropdown": "简体中文",
                         "use_websearch_checkbox": False,
                         }
            # 发送请求
            chat_gpt_api = shared.chatgpt_api
            try:
                response = requests.post(chat_gpt_api, data=post_data)
                # 获取chatGPT生成的prompt
                prompt = response.json()["assistant"]
            except:
                prompt = "A beautifuall girl in a write dress."

            # 构造写实风格的参数
            parameters = shared.sd_model_parameter_template.get("写实")
...
class ProcessedChat:
    def __init__(self, p: StableDiffusionChatProcessing, images_list, seed=-1, info="", subseed=None, all_prompts=None,
                 all_negative_prompts=None, all_seeds=None, all_subseeds=None, index_of_first_image=0, infotexts=None,
                 comments=""):
        self.images = images_list
        self.prompt = p.prompt
    ...
    def infotext(self, p: StableDiffusionChatProcessing, index):
        return create_infotext(p, self.all_prompts, self.all_seeds, self.all_subseeds, comments=[],
                               position_in_batch=index % self.batch_size, iteration=index // self.batch_size)
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    try:
        ...
        res = process_chat_images_inner(p)
...
def process_chat_images(p: StableDiffusionChatProcessing) -> ProcessedChat:
    stored_opts = {k: opts.data[k] for k in p.override_settings.keys()}
...
def process_chat_images_inner(p: StableDiffusionProcessing) -> Processed:
...
class StableDiffusionProcessingChatTxt2Img(StableDiffusionChatProcessing):
    sampler = None
